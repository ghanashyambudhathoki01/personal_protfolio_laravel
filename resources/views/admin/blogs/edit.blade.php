                    ...
                    <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full dark:bg-zinc-900 dark:border-zinc-700 dark:text-zinc-100" :value="old('title', $blog->title)" required autofocus />
                        ...
                        <div>
                            ...
                            <textarea id="excerpt" name="excerpt" class="mt-1 block w-full border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="3">{{ old('excerpt', $blog->excerpt) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="mt-1 rte-container">
                                ...
                                <input type="hidden" id="content-textarea" name="content" value="{{ old('content', $blog->content) }}">
                            </div>
                        ...
                        <div>
                            ...
                            @if($blog->featured_image)
                                <div class="mt-2 mb-4">
                                    <p class="text-xs text-gray-500 dark:text-zinc-500 mb-2">Current Image:</p>
                                    <img src="{{ asset('storage/' . $blog->featured_image) }}" alt="Preview" class="w-32 h-32 object-cover rounded-lg border border-gray-200 dark:border-zinc-700">
                                </div>
                            @endif
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-zinc-500">Max size 2MB. Leave empty to keep current image.</p>
                            <x-input-error class="mt-2" :messages="$errors->get('featured_image')" />
                        </div>

                        <div class="flex items-center">
                            <input id="is_published" name="is_published" type="checkbox" value="1" class="rounded border-gray-300 dark:border-zinc-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-zinc-900" {{ old('is_published', $blog->is_published) ? 'checked' : '' }}>
                            <x-input-label for="is_published" :value="__('Published')" class="ml-2 dark:text-zinc-400" />
                        </div>
